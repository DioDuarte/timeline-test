...
const Timeline: React.FC<TimelineProps> = ({ items }) => {
    ...
    const adjustInitialDates = (minDate: Date, maxDate: Date, zoom: ZoomLevel) => {
        ...
        switch (zoom) {
            case 'day':
                adjustedMinDate = subDays(minDate, 3); // 30 dias antes
                adjustedMaxDate = addDays(maxDate, 3); // 30 dias depois
                break;
            case 'week':
                adjustedMinDate = subWeeks(minDate, 2); // 20 semanas antes
                adjustedMaxDate = addWeeks(maxDate, 2); // 20 semanas depois
                break;
            case 'month':
                adjustedMinDate = subMonths(minDate, 1); // 12 meses antes
                adjustedMaxDate = addMonths(maxDate, 1); // 12 meses depois
                break;
            default:
